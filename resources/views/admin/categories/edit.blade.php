    ...
    <div class="max-w-2xl mx-auto px-4">
        ...
        <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
            <div class="p-8 border-b border-gray-100 bg-indigo-50/30">
                ...
                <p class="text-gray-500 text-sm mt-1 font-medium">Édition de : <span class="text-indigo-600 font-bold">{{ $category->name }}</span></p>
            </div>
            ...
            <form action="{{ route('admin.category.update', $category->id) }}" method="POST" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-bold text-gray-800" required>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="w-full px-5 py-4 rounded-2xl border border-gray-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-medium text-gray-700">{{ old('description', $category->description) }}</textarea>
                </div>

                <div class="flex items-center justify-between p-5 bg-indigo-50/20 rounded-2xl border border-indigo-100">
                    <span class="font-black text-indigo-900 text-xs uppercase tracking-widest">Statut de visibilité</span>
